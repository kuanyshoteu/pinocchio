!function() {
  ...
  Calendar.prototype.drawDay = function(day) {
    ...
    if(day.format('DD') != currentDay){
        ...
        this.getWeek(day);
        //Outer Day
        day_end = parseInt($('.end').val().split('-')[2])
        month_end = parseInt($('.end').val().split('-')[1])
        year_end = parseInt($('.end').val().split('-')[0])
        day_start = parseInt($('.start').val().split('-')[2])
        month_start = parseInt($('.start').val().split('-')[1])
        year_start = parseInt($('.start').val().split('-')[0])
        start_date = day_start + month_start * 100 + year_start * 10000
        end_date = day_end + month_end * 100 + year_end * 10000

        var cell_month = parseInt(day.format('MM'))
        var cell_year = parseInt(day.format('YYYY'))
        var cell_day = parseInt(day.format('DD'))
        cell_date = cell_day + cell_month * 100 + cell_year * 10000;
        ...
        if(cell_date == start_date){
            number.setAttribute('style', 'background-color: #21BA45;')
            outer.setAttribute('classcopy', outer.getAttribute('class'))
            outer.setAttribute('class', outer.getAttribute('class') + ' start_cell')
        }
        if(cell_date == end_date){
            number.setAttribute('style', 'background-color: #DB2828;')
            outer.setAttribute('classcopy', outer.getAttribute('class'))
            outer.setAttribute('class', outer.getAttribute('class') + ' end_cell')
        }
        //Events
        ...
        day_of_week = 'day_of_week' + ((day_counter - 1) % 7).toString();
          if( data.getAttribute(day_of_week) == 'yes' ){
            if( cell_date >= start_date && cell_date <= end_date ){
              //  if(day.format('MM') == cell_month) {
                    outer.setAttribute('style', 'background-color: #5181b8 !important;')
                //}
            }
        }          
  ...
  Calendar.prototype.drawDaySun = function(day) {
    if(day.format('DD') != currentDay){
        ...
        var cell_month = parseInt($('.header_ym').attr('month'))
        var cell_year = parseInt($('.header_ym').attr('year'))
        var cell_day = parseInt(day.format('DD'))
        cell_date = cell_day + cell_month * 100 + cell_year * 10000;
