;$(document).ready(function(){$('.search_group_show').on('click',function(e){$('.hint_students_group').show()})
...
sign='-'}
$('<div class="search_group_link"> <img class="search-group-img" src="'+image+'" alt="photo"> <span class="search-group-name">'+name+'</span> '+check+' <a class="ui button mini blue add_student add_student'+id+' search_group_add" onclick="add_student('+"'"+id+"'"+')" id="'+id+'">'+sign+'</a> </div>').appendTo('.hint_students_group')}}})}});$('.det').on('click',function(e){this_=$(this)
console.log('work0')
...
var instance_id=$('.instance_id').attr('id');$.ajax({url:url,method:"GET",data:{'day':day,'time':time,'checked':checked,'instance_id':instance_id,},success:function(data){location.reload()},error:function(error){console.log('error')}})});$(document).on("click",'.delete_squad_lesson',function(){console.log('bb')
url=$('.delete_lesson_data').attr('url')
id=$(this).attr('id')
$.ajax({url:url,data:{'lecture_id':id,},dataType:'json',success:function(data){$('#squad_lesson'+id).hide('fast');}})});$('.remove_lesson_from_subject').on('click',function(e){var material_id=$(this).attr('material_id');var lesson_id=$(this).attr('lesson_id');var url=$(this).attr('url');$.ajax({url:url,method:"GET",data:{'material_id':material_id,'lesson_id':lesson_id,},success:function(data){$("#lesson_in_material"+material_id+'l'+lesson_id).hide('fast')},error:function(error){console.log('error')}})});$('.open_time_lessons').on('click',function(e){id=$(this).attr('id')
