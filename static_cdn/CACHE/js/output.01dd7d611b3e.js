;$(document).ready(function(){$('.make_zaiavka_new').click(function(e){url='/api/make_zaiavka/'
id=$(this).attr('id')
...
$('.ok_zaiavka').show()
$('.ok_zaiavka_new').show()}}})})
$('.make_zaiavka').click(function(e){course=$(this).attr('course')
...
else{$.ajax({url:url,data:{'title':title,'slogan':slogan,'name':name,'phone':phone,'version':version,},dataType:'json',success:function(data){if(data.ok){$('.success_created').show()
...
$.ajax({url:url,data:{},dataType:'json',success:function(data){for(var i=0;i<data.res.length;i++){$('<tr style="color: #222;"><td>'+data.res[i][0]+'</td><td>'+data.res[i][1]+'</td><td>'+data.res[i][2]+'</td>').appendTo('.history_cont')}}})})
...
$('.wright_review').show()})
$('.save_office_cabinet').click(function(e){url=$(this).attr('url')
id=$(this).attr('id')
title=$('.office_cabinet_create_title'+id).val()
capacity=$('.office_cabinet_create_capacity'+id).val()
$.ajax({url:url,data:{'title':title,'id':id,'capacity':capacity,},dataType:'json',success:function(data){console.log('ffff')
$('<div class="four wide column highlight" style="margin-right: 7px;margin-bottom: 7px;">'+title+' </div>').appendTo('.office_cabinets'+id)}})});$('.save_school_title').click(function(e){url=$(this).attr('url')
id=$(this).attr('id')
status=$(this).attr('status')
if(status=='worktime'){text=$('.school_'+status+'_edit1').val()+'-'+$('.school_'+status+'_edit2').val()}
else{text=$('.school_'+status+'_edit').val()}
$.ajax({url:url,data:{'text':text,'id':id,'status':status},dataType:'json',success:function(data){$('.school_'+status+'_form')
$('.school_'+status+'_form').hide();$('.school_'+status).text(text);}})});$('.login-btn').click(function(e){url='/api/login/'
...
$.ajax({url:url,data:{'username':username,'password':password},dataType:'json',success:function(data){if(data.res=='login'){$('.wrong_login').hide()
location.reload()}
else if(data.res=='error'){$('.wrong_login').show()}}})});$('.register-btn2').click(function(e){url='/api/register/'
name=$('.new_name2').val()
...
else{$('.reg_fill_all').show()}});$('.update_pswd-btn').click(function(e){url='/api/update_pswd/'
...
id=$(this).attr('id')
$.ajax({url:url,data:{'password1':password1,'password2':password2,'id':id,},dataType:'json',success:function(data){location.reload()}})});$('.show_free_cards').click(function(e){url=$(this).attr('url')
checked=$(this).prop('checked')
$.ajax({url:url,data:{'check':checked,},dataType:'json',success:function(data){location.reload()}})});$('.yes_delete_card').click(function(e){url=$('.delete_card_data').attr('url')
id=$('.delete_card_data').attr('id')
$.ajax({url:url,data:{'id':id,},dataType:'json',success:function(data){$('#delete_card_modal').modal('hide');$('#card_container'+id).hide('fast');}})});$('.save_salary').click(function(e){url=$('.salary_url').attr('url')
id=$(this).attr('id')
this_=$(this)
...
$('.add_card').click(function(e){var id=$(this).attr("id")
...
ok=false
for(var i=mail.length-1;i>=0;i--){if(mail[i]=='@'){ok=true;break;}}
if(ok){$.ajax({url:$('.add_card_url').attr('url'),data:{'name':name,'phone':phone,'mail':mail,'comment':comment,'id':id,},dataType:'json',success:function(data){if(data.already_registered){$('.alreadyregistered'+id).show()}
else{location.reload()}}})}
else{$('.wrong_mail_error'+id).show()
console.log('sdfsdf',id)}});$('.crm_option').on('change',function(e){id=$(this).attr('id')
this_=document.getElementById(id);object_id=this_.options[this_.selectedIndex].value;url=$(this).attr('url')
option=$(this).attr('option')
$('.loading').show()
$.ajax({url:url,data:{'object_id':object_id,'option':option,},dataType:'json',success:function(data){$("#calendar").load(document.URL+' #calendar');}});})
...
$("body").click(function(e){$('.new_post_details').hide('fast');});$('.profile_name').click(function(event){$('.profile_links').fadeToggle('fast')})
...
$(document).on("click",'.att_present',function(){var id=$(this).attr('id')
$.ajax({url:$('.attendance_present_url').attr('url'),data:{'id':id,},dataType:'json',success:function(data){$('#grades'+id).show()
$('#attendance'+id).hide()}});})
...
for(var i=0;i<document.getElementsByClassName('switch_att_btn'+lesson_id).length;i++){document.getElementsByClassName('switch_att_btn'+lesson_id)[i].setAttribute('class','switch_att_btn switch_att_btn'+lesson_id)}
for(var i=0;i<document.getElementsByClassName('ppr'+lesson_id).length;i++){document.getElementsByClassName('ppr'+lesson_id)[i].setAttribute('class','paper ppr ppr'+lesson_id)}
...
$('#icondown'+id).hide()
$('#iconright'+id).show()}});$('.add_variant').click(function(event){var id=$(this).attr('id')
...
ul.appendChild(li)})
$('.add_answer').click(function(event){var id=$(this).attr('id')
...
div.appendChild(br)})
$('.add_tag').click(function(event){var id=$(this).attr('id')
...
a.innerHTML=name
div.appendChild(a)})
$(".problem_type").click(function(event){event.preventDefault();var this_=$(this)
var id=this_.attr('id')
$('.'+id+'not_test').fadeToggle()
$('.'+id+'test').fadeToggle()
if($('.problem_type_info').attr('info')=='input'){$('.problem_type_info').attr('info','test')
this_.text('Ввод ответа')}
else{$('.problem_type_info').attr('info','input')
this_.text('Тест')}})
$(".content-markdown").each(function(){var content=$(this).text()
...
$(".open_form_paper").click(function(event){event.preventDefault();$(".create_paper").fadeToggle();})
...
$(document).ready(function(){$(".delete_zaiavka").click(function(e){e.preventDefault()
var this_=$(this)
...
if(squadUrl){$.ajax({url:squadUrl,data:{'paper_id':this_.attr("paper_id"),'squad_id':this_.attr("squad_id"),'isin':this_.attr("isin"),},success:function(data){if(this_.attr("isin")=='yes'){this_.css('background-color','#F2F2F2')
...
$('.'+this_.attr("paper_id")+this_.attr("squad_id")+'squad_results_urok').show()}},error:function(error){console.log('error')}})}})
...
var id=this_.attr('id')
if(pageUrl){$.ajax({url:pageUrl,method:"GET",data:{'id':id,'grade':grade,},success:function(data){$('.grade'+id).removeClass('green')
this_.addClass('green')},error:function(error){console.log('error')}})}})
...
var pageUrl=this_.attr("data-href")
var grade=$('#grade'+this_.attr("name")).val();if(pageUrl){$.ajax({url:pageUrl,data:{'id':this_.attr("name"),'grade':grade},dataType:'json',success:function(data){}});}});})
$(document).ready(function(){$(".check_task_answer").click(function(){var this_=$(this)
var id=this_.attr("id")
var pageUrl=this_.attr("data-href")
var paper_id=this_.attr("paper_id")
var answer=""
var tags=''
for(var i=0;i<document.getElementsByClassName('problem_tag_'+paper_id).length;i++){tags=tags+document.getElementsByClassName('problem_tag_'+paper_id)[i].getAttribute('tag_id')+'&'}
if($(".task_type_"+paper_id).attr("type")=="input"){for(var i=0;i<document.getElementsByClassName('check_task_answer_'+paper_id).length;i++){answer=answer+document.getElementsByClassName('check_task_answer_'+paper_id)[i].value+"&"}}
if($(".task_type_"+paper_id).attr("type")=="test"){for(var i=0;i<document.getElementsByClassName("option_"+paper_id).length;i++){if(document.getElementsByClassName("option_"+paper_id)[i].checked){answer=answer+document.getElementsByClassName("option_"+paper_id)[i].getAttribute("value")+"&"}}}
if(pageUrl){$.ajax({url:pageUrl,data:{'id':this_.attr("id"),'answer':answer,'tags':tags,'parent_id':this_.attr("parent_id"),},dataType:'json',success:function(data){if(document.getElementById('hiscoins')!=undefined){document.getElementById('hiscoins').innerHTML=data.hiscoins;}
if(data.solved==true){document.getElementsByClassName('wrong_answer_alert_'+paper_id)[0].setAttribute('style','display:none')
document.getElementsByClassName(paper_id+'solved')[0].setAttribute('style','margin-right: 10px; font-size: 16px; font-weight: 600; color: green;');document.getElementsByClassName(paper_id+'solved_tick')[0].setAttribute('style','color: green;font-size: 16px; font-weight: 600;')
if(data.action=='plus'){$('#coins_'+paper_id).toggleClass('bubble');$('.check_task_answer_'+paper_id).attr('style','border: 1.5px solid #008100 !important');}}
else{document.getElementsByClassName('wrong_answer_alert_'+paper_id)[0].setAttribute('style','display:block')
$('.check_task_answer_'+paper_id).attr('style','')
document.getElementsByClassName(paper_id+'solved')[0].setAttribute('style','margin-right: 10px; font-size: 16px; font-weight: 600; color: black;');document.getElementsByClassName(paper_id+'solved_tick')[0].setAttribute('style','display:none')}}});}});$(".open_group_details_urok").click(function(event){event.preventDefault();var this_=$(this)
...
$('.'+this_.attr("paper_id")+this_.attr("squad_id")+'squad_results').show()}},error:function(error){console.log('error')}})}})})
$(document).ready(function(){$(".task_answer").change(function(){var this_=$(this)
var pageUrl=this_.attr("data-href")
...
var pageUrl=this_.attr("data-href")
if(pageUrl){$.ajax({url:pageUrl,data:{'id':this_.attr("id"),},dataType:'json',success:function(data){location.reload()}});}});$(document).on("click",'.change_task_text',function(){var this_=$(this)
var pageUrl=this_.attr("data-href")
id=this_.attr("id")
...
var answer=""
var variant=""
if($(".task_type_"+paper_id).attr("type")=="input"){answer=answer+document.getElementsByClassName('change_task_answer_'+paper_id)[0].value+"&"}
if($(".task_type_"+paper_id).attr("type")=="test"){for(var i=0;i<document.getElementsByClassName("option_"+paper_id).length;i++){var old_variant=document.getElementsByClassName("option_"+paper_id)[i].getAttribute("value")
variant=variant+document.getElementsByClassName("variant_value_"+paper_id+'v'+old_variant)[0].value+"&"
if(document.getElementsByClassName("option_"+paper_id)[i].checked){answer=answer+document.getElementsByClassName("variant_value_"+paper_id+'v'+old_variant)[0].value+"&"}}}
if(pageUrl){$.ajax({url:pageUrl,data:{'id':id,'text':text,'cost':cost,'answer':answer,'variant':variant,},dataType:'json',success:function(data){location.reload()}});}});})